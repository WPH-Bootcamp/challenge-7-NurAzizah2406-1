...
const TestimonialsSection = () => {
  return (
    <section id="testimonials" className="py-24 bg-secondary text-white relative">
          ...
          {testimonials.map((item) => {
            ...
            return (
              <div key={item.id} className="relative h-full flex flex-col justify-between">
                ...
                <Card
                  className={`flex flex-col justify-between p-8 rounded-2xl h-full border transition-all duration-500 bg-[#0a0f1d] ${
                    isActive
                    ...
                    <div className="flex items-center gap-1">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          key={i}
                          size={14}
                          className={i < item.rating ? 'text-accent fill-accent' : 'text-slate-800'}
                        />
                      ))}
                    </div>
                    ...
                    <div className="flex items-center gap-3 pt-4 border-t border-slate-900/80">
                      <img
                        ...
                        className={`w-10 h-10 rounded-full object-cover border ${
                          isActive ? 'border-primary/50' : 'border-slate-800'
                        }`}
                      />
                      <div>
                        ...
                        <p className={`text-[11px] font-bold ${
                          isActive ? 'text-primary' : 'text-slate-550'
                        }`}>
